...
export default function GovernmentPortalsGrid() {
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-slate-50 via-white to-sky-50/40 px-4 py-24 font-sans md:px-8">
      {/* Background Decorative Ambient Light Blobs */}
      <div className="pointer-events-none absolute top-12 left-1/2 h-[350px] w-[700px] -translate-x-1/2 rounded-full bg-sky-400/10 blur-[140px]" />
      <div className="pointer-events-none absolute right-10 bottom-10 h-[400px] w-[400px] rounded-full bg-amber-400/10 blur-[120px]" />

      {/* Subtle Grid Pattern Overlay */}
      <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_right,#e2e8f0_1px,transparent_1px),linear-gradient(to_bottom,#e2e8f0_1px,transparent_1px)] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_50%,#000_70%,transparent_100%)] bg-[size:3.5rem_3.5rem] opacity-25" />
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {governmentPortals.map((portal, index) => (
            <m.a
              ...
              className={`group relative flex flex-col justify-between overflow-hidden rounded-3xl border bg-white/90 bg-gradient-to-b p-5 shadow-sm backdrop-blur-xl transition-all duration-300 hover:-translate-y-1.5 ${portal.bgClass} ${portal.glowColor}`}
            >
              {/* Top Accent Line */}
              <div className="absolute top-0 right-0 left-0 h-1 bg-gradient-to-r from-slate-200 via-slate-300 to-slate-200 transition-colors group-hover:from-sky-500 group-hover:to-blue-600" />
